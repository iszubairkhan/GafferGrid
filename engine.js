...
// ============================================================
// DEFAULT SCHEDULE TEMPLATES (CUSTOMIZABLE VIA EDITOR)
// ============================================================
   ...
   openJoshModal() {
    ...
    checkDayForLeave() {
        ...
        if (daySelect && leaveGroup) {
            if (daySelect.value === 'Tuesday' || daySelect.value === 'Thursday') {
                leaveGroup.style.display = 'flex';
            } else {
    ...
    bindDOMEvents() {
        ...
        const daySelect = document.getElementById('modalDaySelect');
        if (daySelect) daySelect.addEventListener('change', () => this.checkDayForLeave());

        // Live Hover & Preview Theme Switching
        const joshBtns = document.querySelectorAll('.josh-btn-option');
        joshBtns.forEach(btn => {
            const val = btn.getAttribute('data-value');
            btn.addEventListener('mouseenter', () => this.applyTheme(val));
        ...
        if (joshForm) {
            joshForm.addEventListener('submit', (e) => {
                e.preventDefault();
                ...
                setTimeout(() => {
                    ...
                    this.loadStateForDate();
                    this.updateUI();
                    this.notify(`Auto-configured for ${this.activeDay} (${this.activeDate})!`);
                }, 300);
            });
        }

        const recheckBtn = document.getElementById('recheckBtn');
        if (recheckBtn) recheckBtn.addEventListener('click', () => this.openJoshModal());
        ...
        if (resetBtn) resetBtn.addEventListener('click', () => this.resetDay());

        const exportBtn = document.getElementById('exportDbBtn');
        if (exportBtn) exportBtn.addEventListener('click', () => this.exportDatabaseFile());

        const importBtn = document.getElementById('importDbBtn');
        const fileInput = document.getElementById('dbFileInput');
        if (importBtn && fileInput) {
            importBtn.addEventListener('click', () => fileInput.click());
            fileInput.addEventListener('change', (e) => this.importDatabaseFile(e));
        }
    }
...
bindWeeklyEvents() {
    ...
    if (weeklyBtn && weeklyModal) {
        weeklyBtn.addEventListener('click', () => {
            this.generateWeeklyReport();
            weeklyModal.classList.add('active');
        });
    }
    if (closeWeeklyBtn && weeklyModal) closeWeeklyBtn.addEventListener('click', () => weeklyModal.classList.remove('active'));

    if (copyReportBtn) {
        copyReportBtn.addEventListener('click', () => {
            const reportText = document.getElementById('readableReportText').innerText;
            navigator.clipboard.writeText(reportText).then(() => this.notify('Weekly report copied to clipboard!'));
        });
    }

    if (resetWeeklyBtn) {
        resetWeeklyBtn.addEventListener('click', () => {
            if (confirm("Are you sure you want to clear execution database logs?")) {
                localStorage.removeItem(this.storageKey);
                if (this.cards) this.cards.forEach(card => card.classList.remove('completed'));
                this.generateWeeklyReport();
                this.updateUI();
    ...
    if (savePdfBtn) {
        savePdfBtn.addEventListener('click', () => this.printReportAsPdf('Weekly Performance Report', document.getElementById('readableReportText').innerText));
    }
    if (saveDocxBtn) {
        saveDocxBtn.addEventListener('click', () => this.downloadReportAsDocx(`Weekly_Report_${this.activeDate}`, document.getElementById('readableReportText').innerText));
    }
    if (savePhotoBtn) {
        savePhotoBtn.addEventListener('click', () => this.downloadReportAsImage(`Weekly_Report_${this.activeDate}`, document.getElementById('readableReportText').innerText, 'ExecutionOS Weekly Snapshot'));
    }
    ...
    if (monthlyBtn && monthlyModal) {
        monthlyBtn.addEventListener('click', () => {
            this.generateMonthlyReport();
            monthlyModal.classList.add('active');
        });
    }
    if (closeMonthlyBtn && monthlyModal) closeMonthlyBtn.addEventListener('click', () => monthlyModal.classList.remove('active'));

    if (copyMonthlyBtn) {
        copyMonthlyBtn.addEventListener('click', () => {
            const reportText = document.getElementById('readableMonthlyReportText').innerText;
            navigator.clipboard.writeText(reportText).then(() => this.notify('Monthly report copied!'));
        });
    }
    if (savePdfMonthlyBtn) {
        savePdfMonthlyBtn.addEventListener('click', () => this.printReportAsPdf('Monthly Performance Report', document.getElementById('readableMonthlyReportText').innerText));
    }
    if (saveDocxMonthlyBtn) {
        saveDocxMonthlyBtn.addEventListener('click', () => this.downloadReportAsDocx(`Monthly_Report_${this.activeDate}`, document.getElementById('readableMonthlyReportText').innerText));
    }
    if (savePhotoMonthlyBtn) {
        savePhotoMonthlyBtn.addEventListener('click', () => this.downloadReportAsImage(`Monthly_Report_${this.activeDate}`, document.getElementById('readableMonthlyReportText').innerText, 'ExecutionOS Monthly Snapshot'));
    }
    if (resetMonthlyBtn) {
    resetMonthlyBtn.addEventListener('click', () => {
        if (confirm("Are you sure you want to clear all monthly execution database logs?")) {
            localStorage.removeItem(this.storageKey);
            if (this.cards) this.cards.forEach(card => card.classList.remove('completed'));
            this.generateMonthlyReport();
            this.updateUI();
            this.notify('Monthly execution logs reset successfully!');
        }
    });
}
}
...
generateMonthlyReport() {
    ...
    let textReport = `========================================\n`;
    textReport += `   MONTHLY EXECUTION & MASTERY REPORT (30 DAYS)\n`;
    textReport += `   Generated On: ${this.activeDate}\n`;
    textReport += `========================================\n\n`;
    ...
    if (recent30Dates.length === 0) {
        ...
        recent30Dates.forEach(d => {
            ...
            for (let routine in dayEntry) {
                ...
                textReport += `   • Routine : ${routine}\n`;
                textReport += `   • Josh    : ${item.josh}\n`;
                textReport += `   • Score   : ${item.completedCount}/${item.totalCount} Tasks (${item.scorePercent}%) [${statusSymbol}]\n`;
                textReport += `----------------------------------------\n`;
            }
        });
        ...
        textReport += `   • Days Logged           : ${recent30Dates.length} Days\n`;
        textReport += `   • Total Tasks Crushed   : ${totalTasksCompleted} / ${totalTasksScheduled}\n`;
        textReport += `   • Monthly Consistency   : ${overallPercent}%\n`;
        textReport += `   • Trajectory Status     : ${overallPercent >= 75 ? '🔥 High Momentum (Target >8.5 CGPA & Career Growth)' : '⚡ Needs Daily Push'}\n`;
        textReport += `========================================\n`;
    }
...
// Export Helpers for PDF, DOCX, and Canvas Image
printReportAsPdf(title, text) {
    const printWindow = window.open('', '', 'width=800,height=600');
    printWindow.document.write(`
        <html><head><title>${title}</title>
        <style>body { font-family: monospace; padding: 24px; white-space: pre-wrap; background: #fff; color: #000; font-size: 13px; }</style>
...
downloadReportAsImage(filename, text, headerTitle) {
    ...
    lines.forEach((line, index) => {
        ctx.fillText(line, 24, 60 + (index * lineHeight));
    });
    ...
    generateWeeklyReport() {
        ...
        let textReport = `========================================\n`;
        textReport += `   WEEKLY EXECUTION & DISCIPLINE REPORT\n`;
        textReport += `   Runway to Sem 1 Exams (Dec 4)\n`;
        textReport += `========================================\n\n`;
        ...
        if (recentDates.length === 0) {
            ...
            textReport += `   • Total Tasks Completed : ${totalTasksCompleted} / ${totalTasksScheduled}\n`;
            textReport += `   • Overall Consistency   : ${overallPercent}%\n`;
            textReport += `   • Verdict               : ${overallPercent >= 75 ? '🔥 Target 8.5+ CGPA & Career Goals ON SCHEDULE' : '⚡ Keep pushing daily consistency'}\n`;
            textReport += `========================================\n`;
        }
    ...
    exportDatabaseFile() {
        const db = JSON.parse(localStorage.getItem(this.storageKey) || '{}');
        const exportObj = {
            appName: "GafferGrid Routine Tracker",
        ...
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        const filename = `RoutineLog_${this.activeDate}_Backup.json`;
        a.href = url;
        a.download = filename;
        document.body.appendChild(a);
        a.click();
    ...
    importDatabaseFile(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (data.records) localStorage.setItem(this.storageKey, JSON.stringify(data.records));
                else localStorage.setItem(this.storageKey, JSON.stringify(data));
                this.loadStateForDate();
                this.updateUI();
                this.notify('Database imported and synced!');
            } catch (err) {
    ...
    notify(msg) {
        if (this.statusMsg) {
            ...
            setTimeout(() => {
                if (this.statusMsg.textContent === msg) this.statusMsg.textContent = '';
            }, 4000);
        }
    }
}
...
// ============================================================
// SCHEDULE EDITOR STUDIO CONTROLLER (edit.html)
// ============================================================
class ScheduleEditor {
    ...
    init() {
        ...
        this.routineSelect.addEventListener('change', () => {
            this.currentRoutineKey = this.routineSelect.value;
            this.renderEditorTasks();
        });

        if (this.addNewBtn) {
            this.addNewBtn.addEventListener('click', () => this.addNewTask());
        }
        ...
        if (this.resetDefaultBtn) {
            this.resetDefaultBtn.addEventListener('click', () => this.resetToDefaults());
        }
        ...
        tasks.forEach((t, index) => {
            ...
            <div class="edit-task-item" data-index="${index}">
                <div>
                    ...
                    <input type="text" class="item-field-input task-time-input" value="${t.time}" placeholder="e.g. 09:00 AM – 12:00 PM">
                </div>
                <div>
                    <label class="form-label" style="font-size:0.7rem;">Tag / Category</label>
                    <input type="text" class="item-field-input task-tag-input" value="${t.tag}" placeholder="e.g. Deep Work">
                </div>
                <div>
                    ...
                    <input type="text" class="item-field-input task-desc-input" value="${t.desc}" placeholder="Task Action Description">
                </div>
    ...
    addNewTask() {
        ...
        <div class="edit-task-item new-item">
            ...
            <div>
                <label class="form-label" style="font-size:0.7rem;">Tag / Category</label>
                <input type="text" class="item-field-input task-tag-input" value="Custom Task" placeholder="Tag">
            </div>
            <div>
                <label class="form-label" style="font-size:0.7rem;">Title & Action Description</label>
                <input type="text" class="item-field-input task-title-input" value="New Skill Practice" placeholder="Task Title" style="margin-bottom: 4px; font-weight:700;">
                <input type="text" class="item-field-input task-desc-input" value="Custom session description." placeholder="Description">
            </div>
            <div class="item-actions-btn">
    ...
    saveAllChanges() {
        ...
        items.forEach((item, i) => {
            ...
            updatedList.push({
                id: `custom_${this.currentRoutineKey}_${Date.now()}_${i}`,
        ...
        alert('Schedule successfully saved! Returning to your routine dashboard...');
        window.location.href = 'index.html';
    }

    resetToDefaults() {
        if (confirm('Reset all schedule modifications back to original defaults?')) {
            localStorage.removeItem(this.customSchedulesKey);
            this.renderEditorTasks();
            alert('Reset to default timetable templates!');
        }
    }
}
